...
- You may run Git inspection commands through PowerShell, cmd, or direct `git` invocation when they do not create, delete, stage, commit, reset, checkout, restore, move, edit, fetch, pull, push, or otherwise mutate files, the index, refs, config, submodules, or remotes.
- Allowed examples include `git status`, `git diff`, `git diff --stat`, `git diff --name-status`, `git diff --submodule`, `git submodule status`, `git log`, `git show`, `git branch --show-current`, `git rev-parse`, `git ls-files`, and `git ls-tree`.
- Do not run mutating Git commands such as `git add`, `git rm`, `git mv`, `git commit`, `git reset`, `git restore`, `git checkout`, `git switch`, `git clean`, `git stash`, `git merge`, `git rebase`, `git pull`, `git fetch`, `git push`, `git submodule update`, or `git config`.
- Do not wrap read-only Git inspection with shell operations that write, create, delete, rename, or overwrite files, including PowerShell redirection, `Out-File`, `Set-Content`, `Add-Content`, `New-Item`, `Remove-Item`, `Move-Item`, or equivalent cmd commands.
- If a read-only Git command fails because of sandboxing, request escalation only for the same read-only inspection command.
